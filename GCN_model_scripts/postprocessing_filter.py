...
home_folder = f'data_filtrado_posp/results'

if not os.path.exists(home_folder):
    os.makedirs(home_folder)


def apply_eq_filters(audio: torch.Tensor, sample_rate: int) -> torch.Tensor:
    # +2dB @ 2800Hz Q=3
    ...
    # LPF at 8kHz (Q=0.707 ~ Butterworth)
    audio = torchaudio.functional.lowpass_biquad(
        audio, sample_rate, cutoff_freq=8000, Q=0.707
    )
    return audio
...
# @title Training loop
#set to 160 epochs
max_epochs = 160  # @param {type: "slider", min: 1, max: 250, step: 1}
...
for epoch in tqdm(range(current_epoch, max_epochs), desc="Training Progress"):
    ...
    for batch_idx, (dry, wet) in enumerate(train_loader):
        ...
        pred = apply_eq_filters(pred, sample_rate)  # apply EQ filters
...
with torch.no_grad():
    for step, (dry, wet) in enumerate(
        ...
        pred = model(input, cond)
        pred = apply_eq_filters(pred, sample_rate)  #filter
